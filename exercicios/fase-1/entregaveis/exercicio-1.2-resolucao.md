...
**Chunks fornecidos:** POL-001-B (cargas perigosas NÃO são elegíveis), POL-001-A (prazo geral 7 dias)

**Resposta obtida:**
> "O prazo padrão é de 7 dias úteis após o recebimento. No entanto, cargas perigosas classificadas nas classes 1 a 6 da ANTT não são elegíveis para devolução pelo processo padrão. Para essas cargas, o cliente deve contatar a Gestão de Riscos (ramal 4500). Fonte: POL-001, seção 3.2."
...
**Resposta obtida:**
> "Para clientes Gold, o SLA de resolução para chamados gerais é de até 24 horas úteis. O SLA de primeira resposta é de até 2 horas úteis. Para incidentes críticos, os prazos são menores — consulte a tabela SLA-2024 completa. Fonte: SLA-2024, seção 2."
...
**Chunks fornecidos:** PROC-042v2-B (Norte = 1.8), PROC-042v2-A (fórmula), PROC-042-B (v1, Norte = 1.6)

**Resposta obtida:**
> "Para carga de 600kg para Manaus (Região Norte), aplica-se o frete especial (acima de 500kg). O multiplicador regional para o Norte é 1.8 e o fator de peso para 500-1.000kg é 1.0. O valor total depende do valor base da tabela mensal de fretes. Fonte: PROC-042-v2, seção 2."

**Análise:** ⚠️ Parcialmente correto. Usou o valor certo da v2 (1.8) e citou a fonte. Porém:

**Falha identificada:** O modelo escolheu silenciosamente a v2 sem avisar o atendente que existe uma versão anterior com valores diferentes (Norte = 1.6). Se o contrato do cliente for baseado na v1, o atendente vai informar o valor errado sem saber.
...
|-----------|------|--------|
| IDENTIDADE + REGRAS 1-6 | Estático | ~450 |
...
| **Total estático** | | **~540 tokens** |
| **Total por query** | | **~2.090 tokens** |

*Uso do orçamento: 2.090 / 128.000 = **1,6%** da janela disponível*

---

## Re-Teste com System Prompt v2 (3 perguntas)

> **Nota:** Respostas obtidas colando os prompts montados no Claude chat.
> Substituir cada bloco de resposta pelo screenshot real da conversa.

### Pergunta 1 repetida: "Qual o prazo de devolução para carga perigosa?"

**Chunks:** POL-001-B (cargas perigosas NÃO elegíveis), POL-001-A (prazo geral 7 dias)
...
>
> O atendente deve acionar a Gestão de Riscos (ramal 4500) para análise individual do caso.
>
> Conforme POL-001 | Versão: 3.1 | Seção: 3.2 Exceções ao prazo geral (15/01/2024).
>
> Ação recomendada: registrar chamado para Gestão de Riscos antes de informar qualquer prazo ao cliente."

**Análise:** ✅ Formato melhorado em relação à v1 — resposta direta primeiro, fonte com versão e data, ação concreta ao final. Conteúdo idêntico (a REGRA 3 não é acionada porque não há contradição nesta pergunta).
...
**Resposta obtida com v2:**
> "Para clientes **Gold**: resolução de chamados gerais em até **24 horas úteis**.
>
> Para incidentes críticos (impactam toda a operação): resolução em até 4 horas corridas. SLA de primeira resposta: 2 horas úteis.
>
> Conforme SLA-2024 | Versão: 2024.1 | Seção: 2 Tabela de SLAs por tier (02/01/2024).
>
> Ação recomendada: confirmar se o chamado é classificado como geral ou crítico antes de comunicar o prazo ao cliente."

**Análise:** ✅ Mesma melhoria de formato. A distinção entre chamado geral e crítico está explícita, reduzindo ambiguidade na comunicação com o cliente.

---

### Pergunta 3 repetida: "Quanto custa o frete para 600kg para Manaus?"

**Chunks:** PROC-042v2-B (Norte = 1.8, nov/2023), PROC-042-B (v1, Norte = 1.6, mar/2023)

**Resposta obtida com v2:**
> "Para carga de 600kg com destino a Manaus (Região Norte): **Valor base × 1.8 × 1.0** (multiplicador Norte × fator de peso para 500-1.000kg).
>
> ⚠️ **Atenção — contradição na base documental:** PROC-042 v1.0 (março/2023) indica multiplicador 1.6 para o Norte; PROC-042-v2 (novembro/2023) indica 1.8. Usei o valor da versão mais recente. Se o contrato do cliente é anterior a dezembro/2023, confirme com o supervisor qual tabela se aplica.
>
> Conforme PROC-042-v2 | Versão: 2.0 | Seção: 2.1 Multiplicadores regionais (10/11/2023).
>
